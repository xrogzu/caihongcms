...
import com.caihong.cms.manager.main.ApiAccountMng;
import com.caihong.cms.manager.main.ApiRecordMng;
import com.caihong.cms.manager.main.ApiUserLoginMng;
import com.caihong.cms.manager.main.CmsThirdAccountMng;
import com.caihong.cms.service.ImageSvc;
import com.caihong.common.security.encoder.PwdEncoder;
import com.caihong.common.util.AES128Util;
...
public class UserApiAct {
		...
		if(errors.hasErrors()){
			...
			if(record!=null){
				...
				if(user==null){
					...
					user=cmsUserMng.registerMember(username, email,telphone, loginPassword, ip, null,null,null,disabled,1,null,null,null,null,0,0,null,userExt,attrs,config.getMemberConfig().getRegisterSendGrain());
	...
	private String thirdLoginGetSessionKey(ApiAccount apiAccount,
		...
		if(thirdAccount!=null){
			...
			CmsUser user=cmsUserMng.registerMember(username, null,null, thirdKey, ip, null,null,null,disabled,1,null,null,null,null,0,0,null,userExt,null,config.getMemberConfig().getRegisterSendGrain());
			cmsWebserviceMng.callWebService("false",username, thirdKey, null,null,null, config.getMemberConfig().getRegisterSendGrain(),userExt,CmsWebservice.SERVICE_TYPE_ADD_USER);
			//绑定新建的用户
	...
	private void initWeiXinJsCode2SessionUrl(){
		if(getWeiXinJsCode2SessionUrl()==null){
			setWeiXinJsCode2SessionUrl(PropertyUtils.getPropertyValue(
					new File(realPathResolver.get(com.caihong.cms.Constants.JEECMS_CONFIG)),WEIXIN_JSCODE_2_SESSION_URL));
		}
	}
